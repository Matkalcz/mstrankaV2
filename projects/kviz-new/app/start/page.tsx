// app/start/page.tsx — Jednotná veřejná adresa kvízu (/start)
'use client'

import { useState, useEffect, useRef } from 'react'  // useRef needed for currentQuizIdRef
    ...
    const poll = async () => {
      try {
        ...
        if (state?.quizId) {
          ...
          // Načti kvíz a šablonu pouze pokud se quizId změnilo
          if (state.quizId !== currentQuizIdRef.current) {
            ...
            if (qData.template_id) {
              setTmpl(null)
              fetch(`/api/templates/${qData.template_id}`)
                .then(r => r.json())
